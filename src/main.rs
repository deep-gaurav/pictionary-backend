...
async fn main() {
    ...
    let wshandle = wsf.and(with_context).map(|ws: Ws, context| {
        ws.on_upgrade(move |socket| user_connected(socket, context))
    }).with(logg);

    warp::serve(wshandle).run(([127,0,0,1],3030)).await;
}
                    ...
                    match bincode::deserialize(msg.as_bytes()){
                        Ok(message)=>{
                            match message{
                                ...
                                _=>{
                                    warn!("First message not initialize, closing connection {:#?}",msg);
                                    if let Err(e)=tx.send(
                                        Ok(
                                            Message::close_with(CloseCodes::WrongInit as u8,CloseCodes::WrongInit.to_string())
                                        )
                                    ){
                                    ...
                                    match message{
                                        PlayerMessage::CreateLobby=>{
                                            ...
                                            let lobbyid:String = rand::thread_rng()
                                                ...
                                                .take(10)
                                                .collect::<String>(); 
                                            let privatelobbies = &mut context.write().await.private_lobbies;
                                            if let Some(_lob) = privatelobbies.get(&lobbyid){
                                                error!("Lobby exist with id {:#?} {:#?}, returning error",lobbyid,_lob);
                                                player.close(
                                                    CloseCodes::CantCreateLobby
                                                );
                                            }else{
                                                player.status=PlayerStatus::JoinedLobby(lobbyid.clone());
                                        ...
                                        PlayerMessage::JoinLobby(lobbyid)=>{
                                            ...
                                            if let Some(lobby)=privatelobbies.get_mut(&lobbyid){
                                                player.status=PlayerStatus::JoinedLobby(lobby.id.clone());
                                                lobby.add_player(player.clone());
                                                info!("Player {:#?} joined lobby {:#?}",player,lobby);
                                                player.send(
                                                    SocketMessage::LobbyJoined(lobby.clone())
                                                );
                                            }else{
                                                player.close(
                                                    CloseCodes::CantLoinLobbyDoestExist
                                                )
                                            }
                                        }
                                        _=>{}
                    ...
                    Ok(message)=>{
                        if message.is_close(){
                            ...
                            match bincode::deserialize(message.as_bytes()){
                                Ok(player_msg)=>{
                                    player_message(&player.id, &lobbyid, &context, player_msg).await;
                                }
                                Err(er)=>{
...
async fn player_message(player_id:&str,lobbyid:&str,context:&Context,message:PlayerMessage){
    ...
    if let Some(lobby)=lobbies.get_mut(lobbyid){
        ...
        error!("Lobby id {:#?} not found for player id {:#?}",lobbyid,player_id);
    }
